...
functions {
    ...
    #include ../functions/piecewise_exponential.stan
}
data {
    int<lower=1> n_bins;
...
generated quantities {
    real mu_div = mu_div_piecewise_exponential(Ne_c, Ne_a, t0, alpha, mutation_rate,
                                               n_quad, gl_nodes, gl_weights);
    vector[n_bins] mu_ld = correct_ld_finite_sample(
        mu_ld_piecewise_exponential(Ne_c, Ne_a, t0, alpha, left_bins, right_bins,
                                    n_quad, gl_nodes, gl_weights),
        sample_size
    );
}
