...
with ``alpha = log_fold_change / t0`` and ``log_fold_change = log_Ne_c - log_Ne_a``.

The default parameterisation uses ``(log_Ne_c, log_Ne_a, log_t0)`` as independent
parameters to avoid the high correlation between parameters.
"""
...
class PiecewiseExponentialDemography:
    """Bayesian inference of (Ne_c, Ne_a, t0) under a piecewise-exponential model.

    Ne(t) = Ne_c * exp(-alpha * t) for t < t0, Ne_a for t >= t0,
    with alpha = (log Ne_c - log Ne_a) / t0.
    ...
    def _compile(self, code: str):
        ...
        stan_file = self._tmpdir / "piecewise_exponential.stan"
    ...
    def _mc_eval(
        ...
        mc_pi_reps, mc_ld_reps = montecarlo.expected_piecewise_exponential(
            ne_c,
            ne_a,
            t0,
            alpha,
