...
def _():
    ...
    import cmdstanpy
    import bayesld
    import bayesld.montecarlo2 as mc2
...
def _(mo):
    ne_slider = mo.ui.slider(
        start=10, stop=5000, step=10, value=10,
        label="True Ne", show_value=True,
    )
    ne_slider
    return (ne_slider,)
...
def _(Path, cmdstanpy, mo):
    ...
    with mo.status.spinner("Compiling Stan models..."):
        ...
        model_gp = cmdstanpy.CmdStanModel(stan_file=str(_stan_dir / "gp_surrogate_constant_ne.stan"))
...
def _(
    ...
    return eval_points, iteration_log


@app.cell
def _(base_stan_data, eval_points, mo, model_gp, np):
    _final_data = {
        **base_stan_data,
...
def _(az, fit_nuts, plt):
    ...
    az.plot_trace(idata, var_names=["Ne", "gp_rho", "gp_alpha", "gp_bias"], combined=False)
...
def _(Ne_truth, eval_points, fit_nuts, iteration_log, np, plt):
    ...
    for _log in iteration_log:
        ...
        ax_draws.scatter(
            ...
            _log["ne_draws"],
            color=_c, alpha=0.75, s=50, label=_label,
        )
    ax_draws.axhline(Ne_truth, color="red", lw=1.5, linestyle="--", label=f"True Ne={Ne_truth}")
    ...
    ax_gp.errorbar(
        _eval_ne, _eval_delta, yerr=_eval_eps,
        fmt="o", color="gray", alpha=0.7, capsize=3, label="Observed δ ± ε",
    )
    ax_gp.scatter(
        _ne_samples, _bias_samp, alpha=0.03, s=10,
        color="purple", label="GP bias at posterior Ne",
    )
    ax_gp.axhline(0, color="k", lw=0.8, linestyle=":")
    ax_gp.axvline(Ne_truth, color="red", lw=1.5, linestyle="--", label=f"True Ne={Ne_truth}")
