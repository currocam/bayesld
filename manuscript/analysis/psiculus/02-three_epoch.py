...
@app.cell
def _(data, inference, mutation_rate, np, recombination_rate, window_length):
    model = inference.PiecewiseConstant(num_epochs=3).with_data(
        ...
        mu_log_ne = np.log(1e5),
        sigma_step=[1, 1],
        sigma_log_ne=1,
        mu_log_t=np.log([100, 1000]),
        sigma_log_t=[1, 1],
        coupling="random_walk"
    )
    return (model,)
...
@app.cell
def _(train):
    trained1 = train(6636638)
    return (trained1,)


@app.cell
def _(train):
    trained2 = train(12376)
    return (trained2,)
...
@app.cell
def _(az, idata):
    az.plot_convergence_dist(idata, var_names=["Ne_values", "t_boundaries"])
    return
...
@app.cell
def _(idata):
    idata.to_netcdf("results/psiculus/three_epoch.nc")
    return


@app.cell
def _(az):
    idata2 = az.from_netcdf("results/psiculus/three_epoch.nc")
    return (idata2,)


@app.cell
def _(az, idata2):
    az.plot_dist(idata2, var_names="t_boundaries")
    return
...
def _(az, idata, model, plt):
    ...
    ax = model.plot_demography(az.extract(idata, num_samples=400, random_seed = 1234), ax = ax)
    ax.axvline(
        (2022-1972) / 2,
        #(2024-1973) / 2,
...
@app.cell
def _(idata2, plot_posterior_predictive):
    plot_posterior_predictive(idata2)
    return
...
@app.cell
def _(az, idata2):
    az.plot_prior_posterior(idata2,var_names="t_boundaries")
    return


@app.cell
def _(az, idata2):
    az.plot_prior_posterior(idata2,var_names="Ne_values")
    return
