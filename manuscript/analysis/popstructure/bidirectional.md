...
def sim(ne1, ne2, m, seed = 1234):
    ...
    demo.add_migration_rate_change(source="pop_1", dest="pop_0", time=0, rate=m)
...
sub_intensity = sp.Matrix(
    [
        ...
        [sym_m, -2*sym_m, sym_m],                    # migrate
        [0, 2 * sym_m, -(sym_lam2 + 2 * sym_m)],                        # coalesce at c
    ]
)
...
```python {.marimo}
fig.savefig("results/popstructure/plot_bidirectional.pdf")
fig.savefig("results/popstructure/plot_bidirectional.pgf")
```