...
# ///
"""
Simulate windows from Oryza sativa using stdpopsim (constant Ne).
...
NUM_WORKERS = 8
CHROMOSOMES = [str(i) for i in range(1, 13)]  # 1..12 autosomes (skip Mt, Pt)

# 5 linear bins from 0.05 cM to 2 cM (in Morgan: 5e-4 to 2e-2)
_edges = np.linspace(5e-4, 2e-2, 6)
left_bins = _edges[:-1]
right_bins = _edges[1:]
...
def main():
    ...
    out_prefix = sys.argv[1] if len(sys.argv) > 1 else "orysat"
